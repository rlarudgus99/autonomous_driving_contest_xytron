...
class LaneDetector():
    ...
    def predict_lane(self):
        ...
        '''
        predicted_lane = self.lane[1] + [-180/max(np.cos(self.angle), 0.75), 0, 180/max(np.cos(self.angle), 0.75)]
    ...
    def update_lane(self, lane_candidates, predicted_lane):
        ...
        for lc in lane_candidates:
            ...
            if idx == 0:
                estimated_lane = [lc, lc + 180/max(np.cos(self.angle), 0.75), lc + 360/max(np.cos(self.angle), 0.75)]
                ...
                if not lc2_candidate:
                    lc2_candidate.append(lc + 180/max(np.cos(self.angle), 0.75))
                if not lc3_candidate:
                    lc3_candidate.append(lc + 360/max(np.cos(self.angle), 0.75))
                for lc2 in lc2_candidate:
                    for lc3 in lc3_candidate:
                        possibles.append([lc, lc2, lc3])

            elif idx == 1:
                estimated_lane = [lc - 180/max(np.cos(self.angle), 0.75), lc, lc + 180/max(np.cos(self.angle), 0.75)]
                ...
                estimated_lane = [lc - 360/max(np.cos(self.angle), 0.75), lc - 180/max(np.cos(self.angle), 0.75), lc]
    ...
    def __call__(self, img, target_lane):
        ...
        if target_lane == 'middle':
            ...
            #self.angle = -0.25
            print('obstacle_left_stanley')
            return self.angle, self.lane[0]*0.9 + self.lane[1]*0.1

        else:
            #self.angle = 0.06
            print('obstacle_right_stanley')
            return self.angle, self.lane[2]*0.9+self.lane[1]*0.1
            #return self.angle, self.lane[0]*0.4
